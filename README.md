...
| 依赖 | 版本 | 说明 |
|---|---|---|
| JDK | 17+ | 本机当前为 1.8,需安装(建议 Temurin 17 或 21) |
| Maven | 3.8+ | 或使用 IDEA 内置 Maven |
| SQL Server | 2016+(Express 免费版即可) | 需可建库权限 |
| Node.js | 18+ | 本机已有 v24 ✓ |
...
- 登录 / 退出 / 当前用户;路由守卫
- 商品档案:分页搜索、新增(编码自动生成 `SKU%06d`)、修改、停用启用 —— **后续模块照此分层模式开发**
...
1. 客户 / 供应商 / 仓库 / 分类档案(复制商品模块模式)
2. 采购入库单 + 应付 + 付款核销(第一张业务单据,打通"审核写库存/台账/应收应付"事务模式)
3. 销售订单 + 出库(含部分发货、限价校验、信用额度预警)
4. 收款核销 + 应收账龄
5. 报表(销售日报、进销存汇总)
