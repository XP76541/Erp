...
│       │       ├── purchase/     # 采购入库单(审核事务)
│       │       ├── inventory/    # 库存服务(UPDLOCK + 移动加权 + 台账)
│       │       └── finance/      # 应付/应收(应收待开发)
│       └── resources/
│           ├── application.yml
...
- **采购管理 · 采购入库单(第一张业务单据,已打通核心事务模式)**
  - 草稿 → 审核状态机:审核用数据库原子抢占(`UPDATE ... WHERE status='DRAFT'`)防并发重复审核
  - 审核单事务内一次完成:逐行写库存(UPDLOCK 行锁 + 移动加权成本)→ 写库存台账(含结存快照)→ 生成应付(`due_date` = 业务日期 + 供应商账期)→ 记操作日志
  - 服务端计算金额(`qty×price` HALF_UP),单号 `PINyyyyMMdd-nnnn` 行锁取号;前端页面含多行明细编辑、审核二次确认
- **财务管理 · 付款核销**
  - 草稿 → 审核状态机;付款单号 `PAYyyyyMMdd-nnnn`,审核原子抢占
  - 审核单事务内完成:逐行核销应付(UPDLOCK 行锁)→ 校验超额 → 更新应付 `paidAmount` 与 `status`(UNSETTLED/PARTIAL/SETTLED)→ 写核销明细→ 操作日志
  - 前端:付款核销页面(供应商联动加载待核应付,多行核销编辑,总额匹配校验,详情展示)
...
1. 销售订单 + 出库(含部分发货、限价校验、信用额度预警;复用 `InventoryService.stockOut`)
2. 收款核销 + 应收账龄(付款核销已实现，应收对称)
3. 收款核销 + 应收账龄
4. 报表(销售日报、进销存汇总)
